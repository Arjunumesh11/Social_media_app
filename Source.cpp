...
void Addfrndscreen(vector<int> *adj, int id_user, int n, vector<person*> &data)
{
	...
	{
		for (size_t i = 0; i < mutualfrnds.size(); i++)
		{
	...
	case 1:
		if (mutualfrnds.empty())
			break;
		for (size_t i = 0; (i < mutualfrnds.size()) && (i < 5); i++)
		{
...
void loginscreen(int id_user, vector<int> *adj, int no_user, vector<person*> &data)
{
	...
	while (1)
	{
		...
		switch (stoi(choice))
		{
			...
			case 3:if (listfrnd(id_user, adj, data))
					{
...
int main()
{
	...
	vector<int> *adj=new vector<int>[n];                            //adjacency matrix showing connections
